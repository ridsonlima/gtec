...
const CONFIG = {
  veiculo:     { base: '/frota',        subtitulo: 'Faturamento mensal de veículos — Frota CDG RENTAL' },
  equipamento: { base: '/equipamentos', subtitulo: 'Faturamento mensal de equipamentos — CDG RENTAL' },
} as const
...
export async function MedicoesListView({ tipo, searchParams }: { tipo: Tipo; searchParams: SP }) {
  ...
  return (
    <div className="space-y-5">
      <TipoSwitcher tipo={tipo} func="medicoes" />
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        {canGestor && (
          <Link href={`${cfg.base}/medicoes/nova`} className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 hover:shadow-md active:scale-[0.98] transition-colors">
      ...
      {medicoes.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
          ...
          {canGestor && <Link href={`${cfg.base}/medicoes/nova`} className="text-sm text-blue-600 hover:underline mt-2 inline-block">Criar primeira medição →</Link>}
        </div>
      ) : (
