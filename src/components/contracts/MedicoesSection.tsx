...
export function MedicoesSection({ contractId, medicoes, partners, canEdit }: Props) {
  const multiPartner = partners.length > 1
  ...
  const [form, setForm] = useState({
    ...
    valorFaturado: '',
    observacoes: '',
    contractPartnerId: partners.length === 1 ? partners[0].id : '',
  })
  const [editForm, setEditForm] = useState<Partial<{ valorProduzido: string; valorAprovado: string; valorFaturado: string }>>({})
  ...
  function getPartner(contractPartnerId: string | null) {
    if (!contractPartnerId) return null
    return partners.find((p) => p.id === contractPartnerId) ?? null
  }

  function getRepasse(m: Medicao) {
    const partner = getPartner(m.contractPartnerId)
    if (partner) return (m.valorFaturado * partner.percentageTotal) / 100
    // fallback: soma de todos os parceiros
    const totalPct = partners.reduce((s, p) => s + p.percentageTotal, 0)
    return (m.valorFaturado * totalPct) / 100
  }

  async function criarMedicao(e: React.FormEvent) {
    e.preventDefault()
    if (multiPartner && !form.contractPartnerId) return
    setLoading(true)
    const res = await fetch(`/api/contracts/${contractId}/medicoes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...form,
        contractPartnerId: form.contractPartnerId || null,
      }),
    })
    setLoading(false)
    if (!res.ok) {
    ...
    setAdding(false)
    setForm((f) => ({ ...f, valorProduzido: '', valorAprovado: '', valorFaturado: '', observacoes: '' }))
    router.refresh()
  }
  ...
  const colSpanTotal = multiPartner ? 11 : 10
      ...
      {adding && (
        <form onSubmit={criarMedicao} className="border border-blue-100 bg-blue-50/30 rounded-lg p-4 space-y-3 mb-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {multiPartner && (
              <label className="block text-sm font-medium text-gray-700 space-y-1 col-span-2 sm:col-span-3">
                <span>Parceiro *</span>
                <select required value={form.contractPartnerId} onChange={setF('contractPartnerId')} className="input">
                  <option value="">Selecione o parceiro desta medição</option>
                  {partners.map((p) => (
                    <option key={p.id} value={p.id}>{p.empresaParceira.name} ({p.percentageTotal}%)</option>
                  ))}
                </select>
              </label>
            )}
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setAdding(false)} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 text-sm rounded-lg hover:bg-gray-50">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-gray-400 border-b border-gray-100">
                ...
                {multiPartner && <th className="text-left pb-2 font-medium">Parceiro</th>}
                ...
                <th className="text-right pb-2 font-medium">Adiant.</th>
                <th className="text-right pb-2 font-medium text-green-700">Líquido</th>
            ...
            <tbody>
              {medicoes.map((m) => {
                ...
                const partner = getPartner(m.contractPartnerId)
                const partnerPct = partner?.percentageTotal ?? partners.reduce((s, p) => s + p.percentageTotal, 0)
                ...
                return (
                  <>
                    <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                      ...
                      {multiPartner && (
                        <td className="py-2.5 pr-3 text-gray-500">
                          {partner ? (
                            <span className="text-xs bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded font-medium">{partner.empresaParceira.name}</span>
                          ) : '—'}
                        </td>
                      )}
                      {isEditing ? (
                        ...
                        <>
                          ...
                          <td className="py-2.5 pr-3 text-right text-blue-600 font-medium" title={`${partnerPct}%`}>{fmt(repasse)}</td>
                          ...
                          <td className="py-2.5">
                            <div className="flex items-center gap-0.5">
                              <button onClick={() => setExpandedId(isExpanded ? null : m.id)} className="p-1 text-gray-300 hover:text-gray-600 rounded">
                    ...
                    {isExpanded && (
                      <tr key={`${m.id}-adiant`}>
                        <td colSpan={colSpanTotal} className="pb-3 pt-1 px-2">
            ...
            <tfoot className="border-t-2 border-gray-200">
              <tr className="text-xs font-semibold text-gray-700">
                <td className="pt-2.5 pr-3 text-gray-500 uppercase text-xs tracking-wide" colSpan={multiPartner ? 2 : 1}>Totais</td>
