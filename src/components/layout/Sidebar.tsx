...
const AREAS = [
  ...
  { name: 'Obras Proprias', href: '/areas/obras-proprias', code: 'OBRAS_PROP' },
  { name: 'Obras Terceirizadas', href: '/areas/obras-terceirizadas', code: 'OBRAS_TERC' },
  { name: 'SESMT e Logistica', href: '/areas/sesmt', code: 'SESMT' },
    ...
    <div className="flex flex-col h-full bg-gray-900 text-gray-100">
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
        ...
        <div>
          <button
            ...
            className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors', 'text-gray-300 hover:text-white hover:bg-gray-800')}
          >
              ...
              {AREAS.map((area) => (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive(area.href) ? 'text-white bg-gray-700' : 'text-gray-400 hover:text-white hover:bg-gray-800'
                  )}
                >
                  {area.name}
                </Link>
        ...
        <NavItem href="/evidencias" icon={<Building2 className="w-4 h-4" />} label="Evidencias" active={isActive('/evidencias')} />

        {isLeadership && (
          <>
            <div className="border-t border-gray-700 my-2" />
            <NavItem href="/admin/usuarios" icon={<Users className="w-4 h-4" />} label="Usuarios" active={isActive('/admin/usuarios')} />
      ...
      <div className="border-t border-gray-700 px-4 py-3">
        ...
        <button
          ...
          className="flex items-center gap-2 w-full px-2 py-1.5 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-gray-800 transition-colors"
        >
...
function NavItem({ href, icon, label, active }: { href: string; icon: React.ReactNode; label: string; active: boolean }) {
  return (
    <Link
      href={href}
      className={cn('flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors', active ? 'text-white bg-gray-700' : 'text-gray-300 hover:text-white hover:bg-gray-800')}
    >
      {icon}
      <span>{label}</span>
    </Link>
