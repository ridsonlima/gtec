...
export default async function EquipamentosAuditoriasPage({ searchParams }: { searchParams: SP }) {
  ...
  function buildHref(params: Record<string, string>) {
    ...
    return `/equipamentos/auditorias${qs ? `?${qs}` : ''}`
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Visitas de campo de equipamentos — CDG RENTAL</p>
        </div>
        {canGestor && (
          <Link href="/equipamentos/auditorias/nova" className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
      ...
      {pendenciasAbertas > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center justify-between gap-4">
          ...
          <Link href="/equipamentos/auditorias/pendencias" className="text-xs text-red-700 font-medium hover:underline flex-shrink-0">
            Ver todas →
          </Link>
        </div>
      )}
      ...
      <div>
        <FiltroSelect
          ...
          value={contratoId}
          otherParams={{ status }}
          baseHref="/equipamentos/auditorias"
      ...
      {visitas.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
          ...
          {canGestor && (
            <Link href="/equipamentos/auditorias/nova" className="text-sm text-blue-600 hover:underline mt-2 inline-block">
        ...
        <div className="space-y-2">
          {visitas.map((v) => {
            ...
            return (
              <Link
                key={v.id}
                href={`/equipamentos/auditorias/${v.id}`}
      ...
      <div className="flex justify-center">
        <Link
          href="/equipamentos/auditorias/pendencias"
