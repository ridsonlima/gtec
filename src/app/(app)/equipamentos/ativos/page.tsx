...
import { Package, Search, LayoutList } from 'lucide-react'
...
export default async function EquipamentosAtivosPage({ searchParams }: { searchParams: SP }) {
  ...
  const where: any = { tipo: 'equipamento' }
  ...
  function buildHref(params: Record<string, string>) {
    ...
    return `/equipamentos/ativos${qs ? `?${qs}` : ''}`
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Equipamentos — CDG RENTAL
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">Gestão de equipamentos locados às obras do grupo</p>
        </div>
        <div className="flex items-center gap-2">
          <PainelAcumulado itens={itensAcumulados} tipo="equipamento" />
          <Link
            href="/equipamentos/por-contrato"
          ...
          {canEdit && <NovoAtivoModal defaultTipo="equipamento" />}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <form method="GET" action="/equipamentos/ativos" className="relative flex-shrink-0">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 w-52"
          />
        </form>
      </div>
      ...
      {ativos.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
          ...
          {!status && !search && canEdit && (
            <p className="text-xs text-gray-400 mt-1">Use o botão "Novo equipamento" para cadastrar o primeiro</p>
          )}
        </div>
      ) : (
        <TabelaAtivos ativos={ativos} canEdit={canEdit} tipo="equipamento" baseHref="/equipamentos/ativos" />
      )}
    </div>
  )
}
