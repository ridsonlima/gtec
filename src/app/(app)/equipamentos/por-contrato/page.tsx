...
export default async function EquipamentosPorContratoPage() {
  ...
  const alocacoes = await prisma.alocacaoAtivo.findMany({
    where: {
      dataFim: null,
      ativo: { tipo: 'equipamento' },
    },
    include: {
      ativo: {
        select: {
          id: true, tag: true, descricao: true, categoria: true,
          numeroserie: true, marca: true, modelo: true,
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link href="/equipamentos/ativos" className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1">
              <ArrowLeft className="w-3 h-3" /> Todos os equipamentos
            </Link>
          </div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {alocacoes.length} equipamento{alocacoes.length !== 1 ? 's' : ''} alocado{alocacoes.length !== 1 ? 's' : ''} em {grupos.length} contrato{grupos.length !== 1 ? 's' : ''} ·{' '}
      ...
      {grupos.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
          ...
          <p className="text-sm text-gray-400">Nenhum equipamento alocado no momento</p>
          <Link href="/equipamentos/ativos" className="mt-3 inline-block text-xs text-blue-600 hover:underline">Ver todos os equipamentos</Link>
        </div>
      ) : (
              ...
              <div key={contrato.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500">
                    <span><span className="font-semibold text-amber-700">{itens.length}</span> equipamento{itens.length !== 1 ? 's' : ''}</span>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100">
                        ...
                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Nº Série</th>
                      ...
                      {itens.map((item) => (
                        <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                          <td className="px-4 py-2.5 whitespace-nowrap">
                            <Link
                              href={`/equipamentos/ativos/${item.ativo.id}`}
                              className="font-mono text-xs font-bold text-amber-700 hover:underline inline-flex items-center gap-1"
                            >
                              {item.ativo.tag}
                          ...
                          <td className="px-4 py-2.5 whitespace-nowrap font-mono text-xs text-gray-400">{item.ativo.numeroserie ?? '—'}</td>
