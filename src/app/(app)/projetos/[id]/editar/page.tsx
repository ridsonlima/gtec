...
export default function EditarProjetoPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/projects/${params.id}`)
      ...
      .then(({ data }) => {
        ...
        setExpectedEndDate(data.expectedEndDate ? data.expectedEndDate.split('T')[0] : '')
        setResponsibleId(data.responsibleUser.id)
        setResponsibleName(data.responsibleUser.name)
        setLoaded(true)
      })
  }, [params.id])
  ...
  if (!loaded) return <div className="p-8 text-sm text-gray-400">Carregando…</div>

  return (
    <div className="max-w-2xl space-y-5">
      <Link href={`/projetos/${params.id}`} className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-gray-600">
        <ChevronLeft className="w-4 h-4" />
        Projeto
      </Link>

      <div>
        <h1 className="text-xl font-bold text-gray-900">Editar Projeto</h1>
      </div>
      ...
      <form onSubmit={submit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
          <Link href={`/projetos/${params.id}`} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">
            Cancelar
          </Link>
