...
export default async function FrotaPendenciasPage({ searchParams }: { searchParams: SP }) {
  ...
  function buildHref(params: Record<string, string>) {
    ...
    return `/frota/auditorias/pendencias${qs ? `?${qs}` : ''}`
  }
        ...
        <div className="space-y-2">
          {pendencias.map((p) => {
            ...
            return (
              <Link
                key={p.id}
                href={`/frota/auditorias/pendencias/${p.id}`}
