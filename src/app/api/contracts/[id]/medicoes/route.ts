...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.medicao.findUnique({
    where: { contractId_competenciaAno_competenciaMes: { contractId: params.id, competenciaAno: ano, competenciaMes: mes } },
  })
  if (existing) return apiError('Já existe uma medição para esta competência', 400)
